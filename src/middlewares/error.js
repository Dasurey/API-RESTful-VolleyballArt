 ...
 */
/**
 * Formatea errores de Firebase Auth a errores HTTP personalizados
 */
    ...
    if (error && error.code && typeof error.code === 'string' && error.code.startsWith('auth/')) {
        switch (error.code) {
            ...
            case 'auth/user-not-found':
                return new AuthenticationError('🚫 Credenciales incorrectas: email o password incorrectos');
            case 'auth/invalid-email':
                return new ValidationError('📧 El email no es válido');
            case 'auth/email-already-in-use':
                return new ConflictError('📧 El email ya está en uso');
            case 'auth/weak-password':
                return new ValidationError('🔒 La contraseña es muy débil (mínimo 6 caracteres)');
            case 'auth/too-many-requests':
                return new RateLimitError('🚫 Demasiados intentos fallidos, intente más tarde');
            default:
...
const globalErrorHandler = (error, req, res, next) => {
    // Mapear errores de Firebase Auth a errores HTTP personalizados
    error = formatFirebaseAuthError(error);
